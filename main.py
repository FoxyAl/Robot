...
#2 задание программных классов
class Obj:
    def __init__(self, x, y, sz=15):
        self.x=x; self.y=y; self.sz=sz
        self.color=(0,0,255)
...
class Robot:
    def __init__(self, x, y, ang = 1, sz=70):
        self.x=x; self.y=y; self.ang=ang; self.sz=sz;
        self.vLin=0; self.vRot=0
        self.coveredDist=0
        self.debugInfo=""
        v0=(sz, 0)
        self.sensors=[
    ...
    def control(self, objs):
        ...
        # набор экспертных правил управления
        if self.x<0 or self.x>WIDTH or self.y<0 or self.y>HEIGHT:
            ...
            rule=1
        elif nn[1]>=nn[0] and nn[1]>=nn[2]: #избегать препятствие
            self.vRot, self.vLin=-0.65, 30
            rule=2
            ...
            self.vRot, self.vLin = 0.65, 30
            rule=4
        elif nn[0]!=0 and nn[1]!=0 and nn[2]!=0:
            self.vRot, self.vLin = 0, -15
            rule=6
...
class Sensor:
    def __init__(self, robot, x, y, r=70):
        self.robot=robot; self.x=x; self.y=y; self.r=r
    def getPos(self):
        return (self.x, self.y)
...
#3 основная логика имитационного моделирования
def main():
    screen = pygame.display.set_mode((WIDTH, HEIGHT))
    timer = pygame.time.Clock()
    fps = 20; tSim=0; tInd=0; nColl=0; nDet=0

    # создание робота и объектов
    r = Robot(130, 130)
    objs=[]
    for i in range(20):
    ...
    while True:
        ...
        # обработка событий интерфейса пользователя
        ...
        # отрисовка робота и объектов
        screen.fill( (255,255,255) )
        r.draw(screen)
        for o in objs:
            o.draw(screen)

        # вывод на экран параметров симуляции и роботв
        ...
        # журналирование в файл параметров симуляции и роботв
